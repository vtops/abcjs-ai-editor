...
⚠️ **注意**：Cloudflare Pages 不支持 `_redirects` 文件，需要在 Dashboard 中配置重定向规则（见下方步骤）。
...
> 
> ⚠️ **重要**：Cloudflare Pages 不支持 `_redirects` 文件，需要在 Dashboard 中配置重定向规则（见下方"配置 SPA 路由"步骤）。
...
⚠️ **必须配置**：Cloudflare Pages 不支持 `_redirects` 文件，需要手动配置重定向规则。
...
3. **重新部署（如果需要）**
   - 如果页面仍然 404，在 **Deployments** 标签页
   - 找到最新的部署，点击 **Retry deployment**

> 💡 这个配置会让所有路由都重定向到 `index.html`，让 React Router 在客户端处理路由。
...
**解决**：
- ⚠️ **重要**：Cloudflare Pages 不支持 `_redirects` 文件
- 必须在 Cloudflare Dashboard → **Settings** → **Functions** → **Redirects** 中手动添加规则：
  - Source: `/*`
  - Destination: `/index.html`
  - Status code: `200`
